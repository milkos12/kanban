            ...
            case 'delete-element': {
                ...
                columns.map((column, index) => {
                    if(column.id === action.columId) {
                        const itemnsFilter = column.items.filter(item => item.id != action.idItem)
                        //columns[index].items = itemnsFilter;
                        console.log("items into current ", columns[index].items)
                        console.log("filter items ", itemnsFilter)
                        //setColumns(columns)
                    }
                })
            }
